...
class MyXapp(xAppBase):
    ...
    def start(self, e2_node_id, kpm_report_style, ue_ids, metric_names, refresh_rate=125):
        report_period = refresh_rate
        granul_period = refresh_rate
...
if __name__ == '__main__':
    ...
    parser.add_argument("--refresh_rate", type=int, default='', help="xApp config file path")
    ...
    ue_ids = list(map(int, args.ue_ids.split(","))) # Expects "0,1,2,..N"
    ...
    # Start xApp.
    myXapp.start(e2_node_id, kpm_report_style, ue_ids, metrics, refresh_rate)
    # Note: xApp will unsubscribe all active subscriptions at exit.
    
    # Command to open a terminal and run echo "Hello World"
